...
```{r import isocodes, include=FALSE}
...
save(df_codes, file = "../clean-data/langcodes.RData")
```
...
## Language-Oriented Data (glottal ejectives, constant-vowel ratios, tones, types, speakers, elevation)

### Add Glottalized Consonants + Consonant-Vowel Ratios 
First we'll combine data about phonological features of the languages. The glottal features dataset contained data on `r dim(df_ejectives)[1]` languages, while the consonant-vowel data only contained features for `r dim(df_cvration)[1]` languages, and tones for only `r dim(df_tones)[1]`. As we only want to work with languages that have a complete set of features, we will exclude missing values, yielding a language feature dataset with 519 records. 

```{r merge ejectives + cvratios, echo=FALSE}
