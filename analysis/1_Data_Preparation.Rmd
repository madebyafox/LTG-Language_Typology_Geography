...
df_hum.long <- df_hum.long %>% rename(Latitude = `Jan 1949`)

# test <-df_hum.long %>% mutate(longitude = ifelse( (Longitude > 180) , (-180) + (Longitude - 180), Longitude)) 
...
df_temp.long <- df_temp.long %>% rename(Latitude = `Jan 1949`)
...
#TODO :: Converting Longitude values into (-180 to 180) format from the Prime meridian format
#df_hum.long <- df_hum.long %>% mutate(longitude = ifelse( (Longitude > 180) , (-180) + (Longitude - 180), Longitude)) #<-- ERROR
#df_temp.long <- df_temp.long %>% mutate(longitude = ifelse( (Longitude > 180) , (-180) + (Longitude - 180), Longitude)) #<-- ERROR
...
df_environment <- merge(df_hum.long, df_temp.long, by.x=c("Latitude", "Longitude"))
by_geolocation <- df_environment
##Save merged long df
