...
```{r clean types, message=none, warning=none}
...
```{r clean diversity, warning=FALSE}
...
```{r environmental load, warning=FALSE} 

#load data
#NOTE: CSV files were too large, so raw data is imported from .RData files constructed from original csv
suppressMessages(load(file="../raw-data/humidity.RData"))
suppressMessages(load(file="../raw-data/temperature.RData"))
...
df_humidity <- df_humidity %>% filter(df_humidity$`1.87500` != 1.87500)
df_temperature <- df_temperature %>% filter(df_temperature$`1.87500` != 1.87500)

#construct column of dates
date_of_observation <- data.frame("month" = rep(1:12, times=72), "year" = rep(1949:2020, each=12))
## making the date columns
date_of_observation <- date_of_observation %>%  slice(rep(1:n(), each=94))
date_of_observation <- date_of_observation %>% filter(month != 11 | year != 2020) %>% filter(month != 12 | year != 2020)

## adding the dated columns to the df_humidity and temp data frames
df_humidity <- cbind(date_of_observation, df_humidity)
df_temperature <- cbind(date_of_observation, df_temperature)
remove(date_of_observation)

## deleting the incomplete year of observation 2020 (It's a bad year anyways)
## (the dataset only contains points until october)
df_humidity <- df_humidity %>% filter(df_humidity$year != 2020)
df_temperature <- df_temperature %>% filter(df_temperature$year != 2020)
...
##Create long form data frames  for humidity
df_hum.long <- df_humidity %>% 
  pivot_longer('0.000000E+00':'358.125',
               names_to = 'Longitude',
               values_to = 'humidity') %>%
  rename(Latitude = `Jan 1949`)


##Create long form data frames  for temperature
df_temp.long <- df_temperature %>% 
  pivot_longer('0.000000E+00':'358.125',
                names_to = 'Longitude',
...
##Average long dfs by coordinates
df_hum.long <- df_hum.long %>% 
  group_by(Latitude, Longitude) %>% 
  summarise(mean_hum = mean(humidity),
            mean_min_hum = mean(min(humidity)),
            mean_max_hum = mean(max(humidity)))

df_temp.long <- df_temp.long %>% 
  group_by(Latitude, Longitude) %>% 
  summarise(mean_temp = mean(temperature),
            mean_min_temp = mean(min(temperature)),
            mean_max_temp = mean(max(temperature)))

## convert long and lat to numeric
df_hum.long <- df_hum.long %>% mutate(
  Latitude = as.numeric(Latitude),
  Longitude = as.numeric(Longitude),
)

## convert long and lat to numeric
df_temp.long <- df_temp.long %>% mutate(
  Latitude = as.numeric(Latitude),
  Longitude = as.numeric(Longitude),
)

#Convert Longitude values into (-180 to 180) format from the Prime meridian format
df_hum.long <- df_hum.long %>% mutate(longitude = ifelse( (Longitude > 180) , (-180) + (Longitude - 180), Longitude)) 
df_hum.long <- df_hum.long %>% rename(Longitude.360 = Longitude, Longitude.180 = longitude)

df_temp.long <- df_temp.long %>% mutate(longitude = ifelse( (Longitude > 180) , (-180) + (Longitude - 180), Longitude)) 
df_temp.long <- df_temp.long %>% rename(Longitude.360 = Longitude, Longitude.180 = longitude)

#Save individual long dfs 
save(df_hum.long, file = "../clean-data/geo_humidity_long.RData")
save(df_temp.long, file = "../clean-data/geo_temperature_long.RData")
...
```{r merge countries }
#TODO AMY TAKE ANOTHER LOOK might need to manually reconcile full-join mimatches 
...
Fortunately our cleaning efforts produced two dataframes organized identically, so we can join them with a simple cbind. 

```{r combine temp humidity}
by_geolocation <- merge(df_hum.long, df_temp.long %>% select(-Longitude.180), by.x=c("Latitude", "Longitude.360"),by.y=c("Latitude", "Longitude.360"))

##Save merged long df
save(by_geolocation, file = "../clean-data/by_geolocation.RData")
```


## COMPLETE MERGE

Last but not least, we need to merge the language and country data together. 

First we add a column to connect the ISO alpha 2 country codes with the written-out country names.
```{r, warning=FALSE }

#import df with mapping from country name to 2 digit country code
suppressMessages(df_country_code <- read_csv("../raw-data/country_code_iso.csv"))
df_country_code <- df_country_code %>% rename(country_name = Name)

#update by_language to separate list of country codes
by_language <- by_language %>%
  separate(countrycodes, c("primary_country", "secondary_country"), "\\s")

#add country_code to languages file, adding country name to by_language
country_language <- merge(by_language, df_country_code, by.x = "primary_country", by.y = "Code", all = FALSE)
```

Now we will combine the new intermediary data frame `country_language` with the by_country df that contains the rest of the country-level data, resulting in a master df we call `by_country_language`.  
```{r}
by_country_language <- merge(country_language, by_country, by.x=c("country_name"), by.y=c("Country"))
```

The _very_ last outstanding task for file merging is to merge the combined country + language file with the geocoded environmental data file. BUT we don't want to do that until after we've first explored and visualized the data.  Time to move on to file 2_Data_Description.Rmd! 
